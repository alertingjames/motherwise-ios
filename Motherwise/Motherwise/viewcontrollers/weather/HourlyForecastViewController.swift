...
class HourlyForecastViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    
    @IBOutlet weak var weatherList: UITableView!

    var data:ForecastData!
    var weathers = [ForecastData.Weathers]()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if self.weathers.indices.contains(index) {
            ...
            DispatchQueue.main.async {
                cell.lbl_time.text = self.getTimeFromTimeStamp(timeStamp: Double(weather.dt))
                cell.lbl_desc.text = weather.weather[0].description.capitalized
                cell.lbl_temp.text = String(weather.main.temp) + " \(self.scale.symbolForScale())"
                cell.lbl_clouds.text = String(weather.clouds.all) + " %"
                cell.lbl_wind.text = String(weather.wind.speed) + " mps"
                cell.lbl_humidity.text = String(weather.main.humidity) + " %"
    ...
    @objc func tappedView(gesture:UITapGestureRecognizer) {
        ...
        let weather = self.weathers[index!]
        gForecastWeatherData = weather
        gDailyForecastWeatherData = nil
    ...
    func getTimeFromTimeStamp(timeStamp : Double) -> String {
        ...
        return String(day) + " " + months[month - 1] + " " + timeStr
    }
    ...
    func warn(weather:ForecastData.Weathers) -> Bool {
        if weather.weather[0].description.contains("thunderstorm")
            ...
            || gWeatherViewController.isDangerousTemperature(temp_max: weather.main.temp_max, temp_min: weather.main.temp_min)
        {
            return true
        }
