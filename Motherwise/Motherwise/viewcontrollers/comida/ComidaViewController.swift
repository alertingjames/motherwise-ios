...
class ComidaViewController: BaseViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    @IBOutlet weak var foodResourceButton: UIButton!
    ...
    override func viewDidLoad() {
        ...
        gComidaViewViewController = self
        ...
        recipeMenu = (UIStoryboard(name: "Comida", bundle: nil).instantiateViewController(withIdentifier: "RecipeMenu") as! RecipeMenu)
        
        lbl_title.text = "comida_connection".localized().uppercased()
        ...
        foodResourceButton.layer.cornerRadius = foodResourceButton.frame.height / 2
        
        recipeButton.titleLabel?.text = "recipes".localized()
        foodResourceButton.titleLabel?.text = "food_resources".localized()
...
//        self.getPosts(member_id: thisUser.idx)
    ...
    override func viewWillAppear(_ animated: Bool) {
        self.getComidaPosts(member_id: thisUser.idx)
    }
...
//                cell.linkViewH.constant = 0
    ...
    func getComidaPosts(member_id:Int64){
        self.showLoadingView()
        APIs.getComidaPosts(member_id: member_id, handleCallback: {
...
//        let vc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(identifier: "PostDetailViewController")
    ...
    @IBAction func openRecipeMenu(_ sender: Any) {
        self.recipeMenu.view.frame = CGRect(x: 0, y: 0, width: self.screenWidth, height: self.screenHeight)
        UIView.animate(withDuration: 1.2) {
            self.recipeMenu.containerView.alpha = 1
            self.addChild(self.recipeMenu)
            self.view.addSubview(self.recipeMenu.view)
        }
    }
    
    @IBAction func toFR(_ sender: Any) {
        let vc = UIStoryboard(name: "Comida", bundle: nil).instantiateViewController(identifier: "FoodResourcesVC")
        vc.modalPresentationStyle = .fullScreen
